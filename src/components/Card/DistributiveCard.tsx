...
const DistributiveCardRevised = () => {
  const choosenAhli = useDistributiveStore((state) => state.activeColumn);
  const row = useMemo(() => choosenAhli.row + 1, [choosenAhli]);
  const col = useMemo(() => choosenAhli.col + 11, [choosenAhli]);
  const setChoosenAhli = useDistributiveStore((state) => state.setActiveColumn);
  const tableState = useDistributiveStore((state) => state.tableState);
  const setTableState = useDistributiveStore((state) => state.setTable);
    ...
    <div className="w-1/3 flex flex-col items-center gap-10">
      ...
      <CheckerButton
        ...
        onClick={() => {
          setIsAnswered(true);
          if (mult[0] && mult[1]) {
            const ans = parseInt(mult[0]) * row + parseInt(mult[1]) * row;
            const test = row * (col + row);
            if (ans == test) {
              rightAudio.play();
              setIsTrue(true);
              const tempTable = [...tableState];
              tempTable[choosenAhli.row][choosenAhli.col] = test;
...
export default DistributiveCardRevised;
